 ...
 */
export function setupBullDashboard(workerPool: IWorkerPool[], port: Number = process.env.BULL_DASHBOARD_PORT as unknown as Number) {
    const app = express();
    ...
    const { addQueue, removeQueue, setQueues, replaceQueues, router } = createBullBoard(bullMQAdapters);
    app.use('/admin/queues', router);

    // Start the Express server
    app.listen(port, () => {
        console.log(`📊 Bull Dashboard is running on http://localhost:${port}/admin/queues`);
    });
